...
result_path = "/home/lary/workwork/PycharmProjects/test00/brojekt_biot/result.txt"
...
#------------------------------------------------------------------------------------------
def path_finder_function(align1, align2): #["str1","str2"],["str1","str2"] global
    if str(align1)+str(align2) in align_dic:
        return align_dic[str(align1)+str(align2)]


    #matrix is len + 1 * len +1
    ...
    def match_cost_function(align1,align2,sub_matrix=b62,sub_head=b62h):
        #2 seq oder viele möglich(consensuns methode)
    ...
    legal_move_matrix = np.empty((len(align1[0])+1,len(align2[0])+1,2),dtype=object)#zeros((shape=3d),dtype)
    for x in range(np.shape(score_matrix)[0]):
        for y in range(np.shape(score_matrix)[1]):
            if x == 0 and y == 0:
                score_matrix[0][0] = 0
                legal_move_matrix[0][0] = ["match",0]
            elif x==0:
                score_matrix[x][y] = gap_cost + variable_gap_cost * (y-1)
                legal_move_matrix[0][y] = ["x_axe",1]
            elif y ==0:
                score_matrix[x][y] = gap_cost +  variable_gap_cost * (x-1)
                legal_move_matrix[x][0] = ["y_axe",1]
            else:
                #match_missmatch
                ...
                if score_match > gap[0]:
                    legal_move_matrix[x][y] = ["match", 0]
                    score_matrix[x][y] = score_match
                elif gap[0] >= score_match:
                    score_matrix[x][y] = gap[0]
                    legal_move_matrix[x][y] = gap[1:]

    #print("seq1,seq2,matrix: ", align1,align2,"\n", score_matrix)
    #print("\nlegal_move:")
    #[print(list([(y[0],y[1]) for y in x])) for x in legal_move_matrix] #was ein krampf
    #result.write("path_matrix: "+ str(align1) + str(align2) + "\n"+ str(score_matrix)+"leagal_move\n"+str([list([(y[0],y[1]) for y in x]) for x in legal_move_matrix]))
    def traceback_function(move_matrix):  # input=np.array 2d
        position = []
    ...
    position = traceback_function(legal_move_matrix)
...
def close_bransh_finder(score_matrix,offset=0):
    ...
    ind = [i for i, x in enumerate(score_matrix[star][len(score_matrix)-(len(score_matrix)+offset):])]
    guide = [star]
